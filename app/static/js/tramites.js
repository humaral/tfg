    ...
    tabla_tramites.innerHTML = data.tramites.map(t =>
        `<tr>
            ...
            <td class="icono-tramites-activo">
                <a href="${urlEditarTramite}?id=${t.id}" class="button-activo-tramites">
                ${t.activo
                    ? `<iconify-icon class="activo-true-tramite" icon="charm:circle-tick"></iconify-icon>`
                    : `<iconify-icon class="activo-false-tramite" icon="charm:circle-cross"></iconify-icon>`}
                </a>
            </td>
        </tr>`
    ).join("");
